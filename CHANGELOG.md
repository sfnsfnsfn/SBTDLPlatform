...
## `v4.0.0-beta.8` (Unreleased — `vision_platfrom` branch)

### 🚀 New Features

- Add V4 Vision Algorithm Platform Workbench (`--platform` flag) with project-based pipeline UI.
- Add six-step pipeline navigation: Data → Label → Train → Evaluate → Infer → Export.
- Add DataWorkspace for dataset management, image import, and split configuration.
- Add TrainWorkspace for training parameter configuration and algorithm selection.
- Add EvaluateWorkspace for selecting runs, running evaluation, and viewing metrics.
- Add ExportWorkspace for ONNX model export and export artifact management.
- Add JobConsole for real-time background task monitoring with cancel support.
- Add ProjectHomeWidget with New Project / Open Project / Recent Projects workflow.
- Add Project Explorer tree view (images, annotations, builds, runs, exports, jobs).
- Add Inspector panel for viewing selected annotation shape properties.
- Add file-based project skeleton (project.json, labels.json, split_manifest.jsonl).
- Add PlatformService, BuildService, TrainService, EvaluateService, ExportService application layer.
- Add platform E2E fixture tests (52 tests, including 8192x8192 CI large sample validation).
- Add CLI `train-worker` subcommand for background training worker subprocess.
- Add `docs/zh_cn/vision_platform_v4_user_guide.md` — complete V4 platform user guide.

### 🐛 Bug Fixes

- Fix DataWorkspace ViewModel not instantiated causing runtime AttributeError (C1).
- Fix EvaluateWorkspace completely disconnected from data flow (C2).
- Fix ExportWorkspace button forever disabled (C3).
- Add InferenceService and InferWorkspace to replace placeholder Infer page (C4).
- Implement real dataset build handler replacing stub dialog (C5).
- Add `parse_and_update_run()` to write training metrics back to Run records (C6).
- Add validation to prevent silently incomplete datasets without image_sources (C7).
- Integrate Run metrics parsing into TrainWorkspace polling on training completion (C8).

### 🛠️ Improvements

- Embed existing LabelingWidget as the Label workspace in the platform pipeline.
- Freeze current canvas performance baseline; no default rendering path changes.
- Consolidate engineering documentation under `docs/superpowers/` and `.planning/`.
- Add `NewProjectDialog` with task type selection (Detection/OBB) and label editor.
- Add `FileImageSource` implementing `LargeImageSource` protocol for dataset builds.
- Add `InferWorkspace` with single-image and batch inference support.
- Wire all workspace data flows through `WorkbenchWindow.set_project()`.
