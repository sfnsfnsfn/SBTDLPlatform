...
class ExportManager:
    def __init__(self, platform_offline: bool = False):
        self.platform_offline = platform_offline
    ...
    def _export_worker(self, weights_path: str, export_format: str):
        try:
            ...
            if missing_packages:
                ...
                # Platform offline mode: never auto-install packages.
                if self.platform_offline:
                    from anylabeling.platform.application.offline_policy import (
                        OfflinePolicy,
                    )

                    policy = OfflinePolicy(offline_mode=True)
                    report = policy.preflight_check(
                        [p.split(">=")[0].split("<")[0].split(">")[0].split("!=")[0].strip()
                         for p in missing_packages]
                    )
                    recommendations = "\n".join(report.recommendations)
                    error_msg = (
                        f"[OFFLINE] Missing required packages for '{export_format}' export: "
                        f"{', '.join(missing_packages)}.\n{recommendations}"
                    )
                    logging.warning(error_msg)
                    self.notify_callbacks("export_log", {"message": error_msg})
