...
def resolve_training_model_path(model, platform_offline: bool = False):
    ...
    # Platform offline mode: never attempt a network download.
    if platform_offline:
        logging.warning(
            "[OFFLINE] Model '%s' not found in weights directory '%s'. "
            "Network download blocked by platform offline policy. "
            "Please manually place the model file in the weights directory.",
            model_path.name,
            weights_dir,
        )
        return None
