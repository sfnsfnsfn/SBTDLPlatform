...
def _export_annotation_to_yolo(
    ...
    """Convert an AnnotationDocument to YOLO format and write to file.

    This replaces the direct use of LabelConverter.custom_to_yolo when a
    platform AnnotationCodec is provided.

    Returns:
        True if the output file is empty (no valid shapes were written),
        False otherwise.
    """
    is_empty = True
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        for obj in doc.objects:
            label_name = obj.attributes.get("label", "")
            if label_name not in classes:
                continue
            class_index = classes.index(label_name)

            if mode == "hbb" and obj.geometry_type == "bbox_xyxy":
            ...
            elif mode == "seg" and obj.geometry_type == "polygon":
                points = obj.geometry
                if len(points) < 3:
                    continue
                norm_points = [
                    (float(p[0]) / image_width, float(p[1]) / image_height)
                    for p in points
                ]
                points_str = " ".join(
                    f"{x:.6f} {y:.6f}" for x, y in norm_points
                )
                f.write(f"{class_index} {points_str}\n")
                is_empty = False

            elif mode == "obb" and obj.geometry_type == "obb_polygon":
                points = obj.geometry
                if len(points) != 4:
                    continue
                coords: list[float] = []
                for p in points:
                    coords.append(float(p[0]) / image_width)
                    coords.append(float(p[1]) / image_height)
...
def create_yolo_dataset(
    ...
    Args:
        ...
        annotation_codec: Optional AnnotationCodec to use instead of
            LabelConverter.  When provided, this function does NOT import
            from anylabeling.views.*.  When None, the legacy LabelConverter
    ...
    def _process_images_batch(
        ...
        for image_file, label_file in image_label_pairs:
            ...
            if label_file and os.path.exists(label_file):
                ...
                if annotation_codec is not None:
                    # Platform path: use codec + internal YOLO export helper
                    with open(label_file, "r", encoding="utf-8") as _lf:
                        _meta = json.load(_lf)
                    img_w = _meta.get("imageWidth", 0)
                    img_h = _meta.get("imageHeight", 0)
                    doc = annotation_codec.load_annotations(
                        label_file, img_w, img_h
                    )
                    _export_annotation_to_yolo(
                        doc, dst_label_path, mode, classes,
                        skip_empty_files=skip_empty,
                    )
                else:
                    # Legacy path: use LabelConverter
    ...
    if task_type == "Classify":
        ...
        if annotation_codec is not None:
            # Platform path: use provided codec (no LabelConverter import)
            converter = None
        elif task_type.lower() == "pose":
