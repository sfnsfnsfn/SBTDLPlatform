...
from anylabeling.views.labeling.viewport import Camera2D
from anylabeling.views.labeling.viewport.render_quality import (
    RenderQuality,
...
class Canvas(
    ...
    camera_view_changed = QtCore.pyqtSignal()
    ...
    CREATE, EDIT = 0, 1
    DEFAULT_RENDER_QUALITY = RenderQuality.NEAREST
    ENABLE_PIXEL_GRID_RENDERING = False
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.enable_pixel_grid_rendering = kwargs.pop(
            "enable_pixel_grid_rendering",
            self.ENABLE_PIXEL_GRID_RENDERING,
        )
        ...
        self.scale = 1.0
        self.render_quality = self.DEFAULT_RENDER_QUALITY
        self.pixmap = QtGui.QPixmap()
        self.provider = None  #: ImageProvider | None — ROI-based rendering when set
        self.camera = None
        self.visible = {}
        self._hide_backround = False
        self.hide_backround = False
        self.h_shape = None
        # Phase 3b: mask brush tool integration
        self._mask_brush_active: bool = False
    ...
    def mouseMoveEvent(self, ev):  # noqa: C901
        ...
        # Phase 3b: mask brush painting
        if self._mask_brush_active and hasattr(self, "parent"):
            parent = self.parent()
            tool = getattr(parent, "_mask_brush_tool", None)
            if tool is not None and tool.is_stroking:
                tool.continue_stroke(pos)
                self.update()
                return
        ...
        # Middle mouse button panning.
        if self._mid_pan_active and self._mid_prev_pos is not None:
            delta = ev.position() - self._mid_prev_pos
            if self.camera is not None:
                self.override_cursor(CURSOR_MOVE)
                self._pan_camera_by_view_delta(delta)
            elif self.pixmap and self.pixmap.width() and self.pixmap.height():
                self.override_cursor(CURSOR_MOVE)
            ...
            self._mid_prev_pos = ev.position()
            self.repaint()
            return
        ...
        if self.drawing():
            ...
            if self._brush_drawing and self.create_mode == "polygon":
                ...
                if (
                    point_dist * self.current_view_scale()
                    >= self.brush_point_distance
                ):
        ...
        for shape in reversed([s for s in self.shapes if self.is_visible(s)]):
            if shape.shape_type == "cuboid" and len(shape.points) == 8:
                index = self.nearest_cuboid_control(
                    shape, pos, self.epsilon / self.current_view_scale()
                )
            ...
            interaction_scale = self.current_view_scale()
            index = shape.nearest_vertex(pos, self.epsilon / interaction_scale)
            index_edge = shape.nearest_edge(
                pos, self.epsilon / interaction_scale
            )
            if index is not None:
                if self.selected_vertex():
                    self.h_shape.highlight_clear()
            ...
            if shape.shape_type in ["point", "line", "linestrip"]:
                nearest_index = shape.nearest_vertex(
                    pos, self.epsilon * 3 / interaction_scale
                )
    ...
    def mousePressEvent(self, ev):  # noqa: C901
        ...
        # Phase 3b: mask brush stroke start
        if (
            self._mask_brush_active
    ...
    def mouseReleaseEvent(self, ev):
        ...
        # Phase 3b: mask brush stroke end
        if (
            self._mask_brush_active
            and ev.button() == QtCore.Qt.MouseButton.LeftButton
        ):
                ...
                if shape.shape_type in ["point", "line", "linestrip"]:
                    if (
                        shape.nearest_vertex(
                            pos, self.epsilon * 3 / self.current_view_scale()
                        )
                        is not None
                    ):
                        hit = True
                ...
                if shape.shape_type in ["point", "line", "linestrip"]:
                    if (
                        ...
                        and shape.nearest_vertex(
                            point, self.epsilon * 3 / self.current_view_scale()
                        )
                        is not None
                    ):
    ...
    def calculate_offsets(self, point):
        ...
        if self.pixmap is not None:
            ...
            left = self.camera.image_width - 1
            right = 0
            top = self.camera.image_height - 1
            bottom = 0
        else:
            return
    ...
    def bounded_move_shapes(self, shapes, pos):
        ...
        if len(shape_types) == 0:
            ...
            if self.out_off_pixmap(o2):
                if self.pixmap is not None:
                    _bw, _bh = self.pixmap.width(), self.pixmap.height()
                elif self.camera is not None:
                    _bw, _bh = self.camera.image_width, self.camera.image_height
                else:
                    _bw, _bh = 0, 0
                pos += QtCore.QPointF(
                    min(0, int(_bw - o2.x())),
    ...
    def _pixel_view_rect(
        ...
        px0, py0 = coordinate_map.image_to_view(
            float(image_x), float(image_y)
        )
        px1, py1 = coordinate_map.image_to_view(
            float(image_x + 1), float(image_y + 1)
        )

        left = max(view_x0, min(view_x0 + target_w, px0))
        right = max(view_x0, min(view_x0 + target_w, px1))
        top = max(view_y0, min(view_y0 + target_h, py0))
        bottom = max(view_y0, min(view_y0 + target_h, py1))
    ...
    def _paint_pixel_grid(
        self, p, x0, y0, x1, y1, clip_w, clip_h,
        view_x0, view_y0, target_w, target_h, coordinate_map
    ):
        """Paint individual image pixels as colored rectangles.

        Tries cached level-0 QPixmap first (fast, no I/O).  Falls back
        to QImageReader from file if the GPU pixmap copy fails (e.g.
        driver pressure on huge textures).
        """
        qimg = self._try_level0_qimage(x0, y0, clip_w, clip_h)
        if qimg is None:
            qimg = self._try_file_qimage(x0, y0, clip_w, clip_h)
        if qimg is None:
            self._pixel_grid_failed = True
            return
        self._pixel_grid_failed = False
        ...
        for iy in range(y0, y1):
            sy = max(0, min(ph - 1, iy - y0))

            for ix in range(x0, x1):
                sx = max(0, min(pw - 1, ix - x0))
                color = qimg.pixelColor(sx, sy)
                rx, ry, rw, rh = self._pixel_view_rect(
                    coordinate_map,
                    ix,
                    iy,
                    view_x0,
                ...
                p.fillRect(_QRect(rx, ry, rw, rh), color)

    def _try_level0_qimage(self, x0, y0, clip_w, clip_h):
        """Try reading visible pixels from cached level-0 QPixmap."""
        from PyQt6.QtCore import QRect
        provider = self.provider
        provider._ensure_pyramid()
        l0 = provider._pyramid[0] if provider._pyramid else None
        if l0 is None:
            return None
        sx0 = max(0, int(x0))
        sy0 = max(0, int(y0))
        sw = min(clip_w, l0.width() - sx0)
        sh = min(clip_h, l0.height() - sy0)
        if sw <= 0 or sh <= 0:
            return None
        cropped = l0.copy(QRect(sx0, sy0, sw, sh))
        if cropped.isNull():
            return None
        qimg = cropped.toImage()
        return qimg if not qimg.isNull() else None

    def _try_file_qimage(self, x0, y0, clip_w, clip_h):
        """Fallback: read visible pixels from original file via QImageReader."""
        from PyQt6.QtCore import QRect
        from PyQt6.QtGui import QImageReader
        reader = QImageReader(self.provider.image_path)
        reader.setAutoTransform(True)
        reader.setClipRect(QRect(x0, y0, clip_w, clip_h))
        reader.setScaledSize(QRect(0, 0, clip_w, clip_h).size())
        qimg = reader.read()
        return qimg if not qimg.isNull() else None

    def _paint_provider_file(self, p, x0, y0, x1, y1, target_w, target_h,
                             view_x0, view_y0):
        """Fallback: read exact pixels from original file via QImageReader."""
        from PyQt6.QtCore import QRect
        from PyQt6.QtGui import QImageReader, QPixmap
        reader = QImageReader(self.provider.image_path)
        reader.setAutoTransform(True)
        reader.setClipRect(QRect(x0, y0, x1 - x0, y1 - y0))
        reader.setScaledSize(QRect(0, 0, target_w, target_h).size())
        qimg = reader.read()
        if not qimg.isNull():
            pixmap = QPixmap.fromImage(qimg)
            p.drawPixmap(round(view_x0), round(view_y0), pixmap)

    def _paint_provider_image(self, p, coordinate_map):
        """Paint visible region via GPU-accelerated pyramid drawPixmap.

        Uses ``p.drawPixmap(target_rect, pyramid_pixmap, source_rect)``
        so the GPU handles scaling.  No CPU copy+scale on the GUI thread.
        """
        import math

        visible = coordinate_map.visible
        img_w = self.provider.image_width
        img_h = self.provider.image_height

        # Clip visible rect to image bounds.
        x0 = max(0, math.floor(visible.x0))
        y0 = max(0, math.floor(visible.y0))
        x1 = min(img_w, math.ceil(visible.x1))
        y1 = min(img_h, math.ceil(visible.y1))

        if x0 >= x1 or y0 >= y1:
            return

        # Viewport footprint of the clipped region.
        view_x0, view_y0 = coordinate_map.image_to_view(
            float(x0), float(y0)
        )
        view_x1, view_y1 = coordinate_map.image_to_view(
            float(x1), float(y1)
        )
        target_w = max(1, round(view_x1 - view_x0))
        target_h = max(1, round(view_y1 - view_y0))
        clip_w = x1 - x0
        clip_h = y1 - y0

        # Pixel-grid mode is opt-in only. Switching into this branch while
        # wheel zooming causes visible white gaps/flashes on some GPU paths.
        # Threshold of 200 means each pixel is >= 6 screen pixels in a
        # 1200px viewport — fine enough that GPU interpolation works below.
        _pixel_grid_max_pixels = 200
        if (
            getattr(
                self,
                "enable_pixel_grid_rendering",
            ...
            and clip_w <= _pixel_grid_max_pixels
            and clip_h <= _pixel_grid_max_pixels
        ):
            self._paint_pixel_grid(
                p, x0, y0, x1, y1, clip_w, clip_h,
                view_x0, view_y0, target_w, target_h, coordinate_map
            )
            if not getattr(self, "_pixel_grid_failed", False):
                return
            self._pixel_grid_failed = False
            # Pixel-grid failed — fall through to file path for exact colors.
            # Skip GPU path (pyramid level 1 would produce streaks/blur).
            self._paint_provider_file(p, x0, y0, x1, y1, target_w, target_h,
                                       view_x0, view_y0)
            return

        # GPU path: drawPixmap with source rect from pyramid pixmap.
        pyramid_pixmap, level, scale = self.provider.get_pyramid_pixmap(
            clip_w, clip_h, target_w, target_h
        )

        if pyramid_pixmap is not None:
            # Map clipped image rect to pyramid-level coordinates.
            # Clamp to actual pixmap dimensions (QImageReader may
            # produce slightly different dimensions than metadata).
            pw = pyramid_pixmap.width()
            ph = pyramid_pixmap.height()
            src_x = max(0, min(pw - 1, int(x0 * scale)))
            src_y = max(0, min(ph - 1, int(y0 * scale)))
            src_x2 = max(src_x + 1, min(pw, int(x1 * scale + 0.5)))
            src_y2 = max(src_y + 1, min(ph, int(y1 * scale + 0.5)))
            src_w = src_x2 - src_x
            src_h = src_y2 - src_y
            if src_w > 0 and src_h > 0:
                from PyQt6.QtCore import QRectF
                p.drawPixmap(
                    QRectF(view_x0, view_y0, target_w, target_h),
                    pyramid_pixmap,
                    QRectF(float(src_x), float(src_y), float(src_w), float(src_h)),
                )
        else:
            # Level 0 not cached — read from original file.
            import time as _time
            _t0 = _time.perf_counter()
            from anylabeling.views.labeling.viewport import RectF

            try:
                result = self.provider.read_region(
                    RectF(float(x0), float(y0), float(x1), float(y1)),
                    (target_w, target_h),
                )
                provider_pixmap = result.image
            except Exception:
                provider_pixmap = None
            _elapsed = (_time.perf_counter() - _t0) * 1000
            try:
                from anylabeling.views.labeling.logger import logger
                logger.info(
                    f"[canvas] Provider read (file fallback): "
                    f"region=({x0},{y0},{x1},{y1}) "
                    f"→ ({target_w},{target_h}) in {_elapsed:.1f} ms"
                )
            except Exception:
                pass
            if provider_pixmap is not None:
                p.drawPixmap(
                    round(view_x0), round(view_y0), provider_pixmap
                )
    ...
    def paintEvent(self, event):  # noqa: C901
        ...
        has_pixmap = (
            ...
            and self.pixmap.width() > 0
            and self.pixmap.height() > 0
        )
        has_provider = getattr(self, "provider", None) is not None

        if not has_pixmap and not has_provider:
            super().paintEvent(event)
            return

        # Image dimensions — use camera when available (works for both
        # pixmap and provider paths), fall back to pixmap for legacy.
        if self.camera is not None:
            _img_w = self.camera.image_width
            _img_h = self.camera.image_height
        else:
            _img_w = self.pixmap.width() if self.pixmap else 0
            _img_h = self.pixmap.height() if self.pixmap else 0
        ...
        apply_render_quality(
            p,
            getattr(self, "render_quality", self.DEFAULT_RENDER_QUALITY),
        )

        coordinate_map = None
        if self.camera is not None:
            coordinate_map = self.camera.coordinate_map()
            Shape.scale = coordinate_map.scale
        else:
            Shape.scale = self.scale

        # --- Draw image ---
        if has_provider and coordinate_map is not None:
            self._paint_provider_image(p, coordinate_map)
        else:
            if coordinate_map is not None:
                p.setTransform(coordinate_map.image_to_view_qtransform())
            else:
                p.scale(self.scale, self.scale)
                p.translate(self.offset_to_center())
            p.drawPixmap(0, 0, self.pixmap)

        # In provider mode the transform must be set for shape drawing
        # (shapes are in image coordinates).  In legacy mode the transform
        # was already set above — setting it again is harmless but we keep
        # the code explicit for clarity.
        if has_provider and coordinate_map is not None:
            p.setTransform(coordinate_map.image_to_view_qtransform())

        # Draw compare view: left side shows compare image, right side shows original
        # split_position: 0 = all original, 1 = all compare
        # Only active in legacy pixmap mode (provider mode disables compare view).
        if (
            not has_provider
        ...
        if self.is_loading and has_pixmap:
        ...
        if self.show_groups:
            ...
            for group_id in grouped_shapes:
                ...
                for shape in shapes:
                    ...
                    pen.setWidthF(1.5)  # fixed screen-pixel width
                    ...
                    triangle_radius = 3  # fixed screen pixels
        ...
        if self.show_linking:
            ...
            for linking in linking_pairs:
                ...
                arrow_size = 8  # fixed screen pixels for arrowhead
                angle = math.atan2(
                    vp[1] - kp[1], vp[0] - kp[0]
        ...
        # Draw mouse coordinates (cosmetic pen — always 1 device pixel regardless of zoom)
        if self.cross_line_show:
            pen = QtGui.QPen(
                QtGui.QColor(self.cross_line_color),
            )
            pen.setStyle(Qt.PenStyle.DashLine)
            pen.setCosmetic(True)  # sub-pixel: always 1 device pixel
            p.setPen(pen)
            p.setOpacity(self.cross_line_opacity)
            p.drawLine(
                QtCore.QPointF(self.prev_move_point.x(), 0),
                QtCore.QPointF(self.prev_move_point.x(), _img_h),
            )
            p.drawLine(
                QtCore.QPointF(0, self.prev_move_point.y()),
                QtCore.QPointF(_img_w, self.prev_move_point.y()),
            )
        ...
        # Draw compare view split line (pixmap mode only)
        if (
            not has_provider
            and self.pixmap is not None
            and self.compare_pixmap is not None
    ...
    def current_view_scale(self):
        if self.camera is not None:
            return self.camera.coordinate_map().scale
        return self.scale

    def init_camera_for_image(self, width, height):
        """Create a Camera2D for an image/world size without changing drawing."""
    ...
    def set_camera_viewport(self, width, height):
        """Update Camera2D viewport size without changing the paint path."""
        self._provider_cache = None  # invalidate — viewport size affects scale
    ...
    def view_to_image_point(self, pos):
        """Map a viewport point to image coordinates through CoordinateMap."""
        view_x, view_y = self._point_xy(pos)
        image_x, image_y = self._require_camera().coordinate_map().view_to_image(
            view_x,
            view_y,
        )
    ...
    def image_to_view_point(self, point):
        """Map an image point to viewport coordinates through CoordinateMap."""
        image_x, image_y = self._point_xy(point)
        view_x, view_y = self._require_camera().coordinate_map().image_to_view(
            image_x,
            image_y,
        )
        return QtCore.QPointF(view_x, view_y)
    ...
    def _require_camera(self):
        if self.camera is None:
            raise RuntimeError("Canvas camera is not initialized")
        return self.camera

    def fit_camera_width(self):
        camera = self._require_camera()
        camera.fit_width()
        self.camera_view_changed.emit()
        return camera

    def set_camera_scale(self, scale, anchor_view_point=None):
        camera = self._require_camera()
        if anchor_view_point is None:
            camera.set_scale_around_center(float(scale))
        else:
            view_x, view_y = self._point_xy(anchor_view_point)
            camera.set_scale_at_view_point(view_x, view_y, float(scale))
        self.camera_view_changed.emit()
        return camera

    def _pan_camera_by_view_delta(self, delta):
        dx, dy = self._point_xy(delta)
        self._require_camera().pan_by_view_delta(dx, dy)
        self.camera_view_changed.emit()
    ...
    def out_off_pixmap(self, p):
        ...
        if self.pixmap is None:
            if self.camera is not None:
                return not (
                    0 <= p.x() <= self.camera.image_width - 1
                    and 0 <= p.y() <= self.camera.image_height - 1
                )
            return True
        w, h = self.pixmap.width(), self.pixmap.height()
    ...
    def intersection_point(self, p1, p2):
        ...
        """
        if self.pixmap is not None:
            _iw, _ih = self.pixmap.width(), self.pixmap.height()
        elif self.camera is not None:
            _iw, _ih = self.camera.image_width, self.camera.image_height
        else:
            return p1
        points = [
            (0, 0),
        ...
        x1 = min(max(p1.x(), 0), _iw - 1)
        y1 = min(max(p1.y(), 0), _ih - 1)
        x2, y2 = p2.x(), p2.y()
        intersections = list(
            self.intersecting_edges((x1, y1), (x2, y2), points)
        )
    ...
    def wheelEvent(self, ev: QWheelEvent):
        ...
        if mods & QtCore.Qt.KeyboardModifier.ControlModifier:
            ...
            # wheel: zoom
            if self.camera is not None:
                factor = 1.1 if delta.y() > 0 else 0.9
                view_x, view_y = self._point_xy(ev.position())
                self.camera.zoom_at_view_point(view_x, view_y, factor)
                self.camera_view_changed.emit()
                self.update()
            else:
    ...
    def load_pixmap(self, pixmap, clear_shapes=True):
        """Load pixmap"""
        self.pixmap = pixmap
        self.provider = None  # clear provider when switching to pixmap mode
        self._provider_cache = None  # invalidate provider paint cache
        if pixmap is not None and not pixmap.isNull():
            self.init_camera_for_image(pixmap.width(), pixmap.height())
        if clear_shapes:
            self.shapes = []
        self.update()

    def load_image_provider(self, provider, clear_shapes=True):
        """Load an ImageProvider for ROI-based rendering.

        Switches the canvas to provider mode: paintEvent reads only the
        currently visible image region instead of drawing a full QPixmap.
        Call ``load_pixmap()`` to switch back to full-image mode.
        """
        self.provider = provider
        self.pixmap = None  # clear pixmap to force provider path in paintEvent
        self._provider_cache = None  # invalidate provider paint cache
        self.init_camera_for_image(provider.image_width, provider.image_height)
        if clear_shapes:
            self.shapes = []
        self.update()
