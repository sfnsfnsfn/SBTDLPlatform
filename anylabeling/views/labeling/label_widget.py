...
# Provider threshold: images larger than this (in megapixels) use
# QImageRegionProvider instead of full QPixmap rendering.
# Set to 0 to disable provider path entirely (forces legacy pixmap path).
PROVIDER_THRESHOLD_MEGAPIXELS = 1_000_000  # disabled: use legacy pixmap path
PROVIDER_TILE_CACHE_BYTES = 256 * 1024 * 1024  # 256 MiB
...
class LabelingWidget(LabelDialog):
    ...
    # Phase 3b: debounced auto-save signals
    auto_save_completed = QtCore.pyqtSignal(bool, str)  # (ok, message)

    # Phase 3b: AI suggestion workflow signals
    suggestion_accepted = QtCore.pyqtSignal(str)  # suggestion_id
    suggestion_rejected = QtCore.pyqtSignal(str)  # suggestion_id

    _AUTO_SAVE_DEBOUNCE_MS = 500

    # V4 Platform: project_context is set by WorkbenchWindow when embedding
    # this widget as the "Label Workspace" page. It is None when running
    # standalone (existing behavior unchanged).
    project_context: dict | None = None

    def set_project_context(self, ctx: dict | None) -> None:
        """Set or clear the platform project context.

        Called by :class:`WorkbenchWindow` when embedding this widget as the
        Label Workspace page.  When *ctx* is None the widget behaves exactly
        as before — existing standalone usage is unchanged.

        Args:
            ctx: A dict with keys like ``project_path``, ``assets_dir``,
                ``annotations_dir``, or None to clear.
        """
    ...
    def __init__(  # noqa: C901
        ...
        # Phase 3b: mask brush tool (lazy-initialized)
        self._mask_brush_tool = None

        # Phase 3b: AI suggestion storage
        self._pending_suggestions: dict[str, list[dict]] = {}
        # keyed by asset_path -> list of suggestion dicts with:
        #   id, model_id, model_name, confidence, shape, status, created_at

        # set default shape colors
        Shape.line_color = QtGui.QColor(*self._config["shape"]["line_color"])
        Shape.fill_color = QtGui.QColor(*self._config["shape"]["fill_color"])
        Shape.fill_color.setAlpha(self._shape_fill_opacity())
        ...
        # Phase 3b: debounced auto-save timer (must be AFTER super().__init__)
        self._auto_save_timer = QtCore.QTimer(self)
        self._auto_save_timer.setSingleShot(True)
        self._auto_save_timer.setInterval(self._AUTO_SAVE_DEBOUNCE_MS)
        ...
        self.label_list.delete_requested.connect(self.delete_selected_shape)
        ...
        self.canvas.camera_view_changed.connect(self._set_camera_manual_zoom)
        ...
        create_mask_brush_mode = action(
            self.tr("Mask Brush"),
            self.toggle_mask_brush_mode,
            shortcuts.get("create_mask_brush", "Ctrl+Shift+M"),
            "mask_brush",
            self.tr(
                "Edit pixel-level mask on selected polygon shape"
            ),
            enabled=False,
        )
        ...
        self.actions = utils.Struct(
            ...
            create_mask_brush_mode=create_mask_brush_mode,
            edit_mode=edit_mode,
    ...
    def menu(self, title, actions=None):
        ...
        main_win = self._find_main_window()
        if main_win is not None:
            menu = main_win.menuBar().addMenu(title)
        else:
            menu = _QtW.QMenu(title, self)
        if actions:
    ...
    def statusBar(self):
        ...
        from PyQt6 import QtWidgets as _QtW
        if not hasattr(self, "_fallback_statusbar"):
            self._fallback_statusbar = _QtW.QStatusBar(self)
        return self._fallback_statusbar
    ...
    def set_dirty(self):
        ...
        # Phase 3b: debounced auto-save when platform-enabled
        if self._auto_save_enabled and self.image_path:
        ...
        # Legacy: immediate auto-save (standalone mode)
    ...
    # ------------------------------------------------------------------
    # Phase 3b: Debounced auto-save
    # ------------------------------------------------------------------

    def enable_auto_save(self) -> None:
        """Enable the debounced auto-save timer.

        Called by LabelWorkspace when embedding this widget.  Once enabled,
        ``set_dirty()`` starts a 500 ms debounce timer instead of saving
        immediately.
        """
        self._auto_save_enabled = True

    def disable_auto_save(self) -> None:
        """Disable the debounced auto-save timer.

        Stops any pending timer and falls back to manual or legacy auto-save.
        """
        self._auto_save_enabled = False
        self._auto_save_timer.stop()

    def _do_auto_save(self) -> None:
        """Auto-save slot fired after debounce timer expires.

        Saves to a ``.tmp`` file and atomically replaces the target ``.json``.
        Emits ``auto_save_completed`` with (ok, message) on completion.
        """
        if not self.image_path or not self._auto_save_enabled:
            return
        ...
        try:
            if self.project_context:
                # Platform mode: save via annotation_adapter with atomic write
                ...
                platform_shapes = list(self.canvas.shapes)
                save_annotations_for_asset(
                    asset_path, platform_shapes,
                    self.project_context["project_path"],
                    self.project_context.get("label_name_to_id", {}),
                )
            else:
                # Standalone mode: use legacy save_labels
                label_file = osp.splitext(self.image_path)[0] + ".json"
                if self.output_dir:
                    label_file_without_path = osp.basename(label_file)
                    label_file = (
            ...
            self.auto_save_completed.emit(
                True,
                self.tr("Auto-saved at %s") % time.strftime("%H:%M:%S"),
            )
        except (OSError, json.JSONDecodeError, ValueError) as exc:
            logger.exception("Auto-save failed")
            self.auto_save_completed.emit(
                False,
                self.tr("Save failed: %s") % exc,
            )

    def _check_recovery_files(self) -> list[str]:
        """Scan for orphaned ``.tmp`` recovery files near current image.

        Returns a list of recoverable ``.tmp`` file paths.
        """
        if not self.image_path:
            return []
        from pathlib import Path
        label_path = Path(osp.splitext(self.image_path)[0] + ".json")
        tmp_path = label_path.with_suffix(".json.tmp")
        recoverable: list[str] = []
        if tmp_path.exists():
            recoverable.append(str(tmp_path))
        return recoverable
    ...
    def _window_title(self):
        ...
        if self.filename is not None:
            ...
            if hasattr(self, "image") and not self.image.isNull():
                image_size = (
                    f" [{self._current_image_width()}x"
                    f"{self._current_image_height()}]"
                )
            title = (
    ...
    def _find_main_window(self):
        """Walk up the parent chain to find the QMainWindow ancestor.

        When embedded in a platform workspace (LabelWorkspace ->
        QStackedWidget -> WorkbenchWindow), the fixed-depth
        ``self.parent.parent`` pattern is wrong. This walks upward
        until it finds a QMainWindow or returns None.
        """
        ancestor = self.parent
        while ancestor is not None:
            if hasattr(ancestor, "menuBar") and hasattr(ancestor, "statusBar"):
                return ancestor
    ...
    def update_progress_title(self):
        main_win = self._find_main_window()
        if main_win is not None:
            main_win.setWindowTitle(self._window_title())
    ...
    def toggle_mask_brush_mode(self):
        """Toggle pixel-level mask brush editing on the selected shape.

        Only enabled when a polygon-type shape is selected (edit mode).
        The mask brush paints directly on the shape's mask bitmap.
        """
        # Lazy-init the tool
        if self._mask_brush_tool is None:
            from anylabeling.views.labeling.widgets.mask_brush import (
                MaskBrushTool,
            )
            self._mask_brush_tool = MaskBrushTool(self.canvas)

        tool = self._mask_brush_tool

        # If already active, deactivate
        if tool.is_stroking:
            tool.end_stroke()
            self.canvas._mask_brush_active = False
            self.canvas.update()
            self.actions.create_mask_brush_mode.setEnabled(True)
            return

        # Activate: must be in edit mode with a polygon shape selected
        selected = self.canvas.selected_shapes
        if not selected:
            self.status(
                self.tr("Select a polygon shape first to use mask brush.")
            )
            return
        shape = selected[0]
        if getattr(shape, "shape_type", "polygon") not in (
            "polygon", "segmentation"
        ):
            self.status(
                self.tr(
                    "Mask brush is only available for polygon shapes."
                )
            )
            return

        # Enter mask brush mode
        tool.attach_to_shape(shape)
        self.canvas._mask_brush_active = True
        self.canvas.update()
        self.actions.create_mask_brush_mode.setEnabled(False)
        self.status(
            self.tr(
                "Mask brush active — paint on canvas. "
                "Click Mask Brush again to finish."
            )
        )
    ...
    def save_attributes(self, _shapes):
        ...
        try:
            ...
            label_file.save(
                ...
                image_height=self._current_image_height(),
    ...
    def _update_shape_color(self, shape):
        ...
        shape.fill_color = QtGui.QColor(r, g, b, self._shape_fill_opacity())
        shape.select_line_color = QtGui.QColor(255, 255, 255)
        shape.select_fill_color = QtGui.QColor(r, g, b, 155)

    def _shape_fill_opacity(self):
        shape_config = self._config.get("shape", {})
        opacity = shape_config.get("fill_opacity")
        if opacity is None:
            fill_color = shape_config.get("fill_color", [])
            opacity = fill_color[3] if len(fill_color) >= 4 else 128
        return max(0, min(255, int(opacity)))

    def _refresh_shape_fill_opacity(self):
        opacity = self._shape_fill_opacity()
        Shape.fill_color.setAlpha(opacity)
        for shape in self.canvas.shapes:
            fill_color = QtGui.QColor(shape.fill_color or Shape.fill_color)
    ...
    def label_selection_changed(self):
        ...
        if self.canvas.editing():
            ...
            drawing_active = self.canvas.drawing() and self.canvas.current
            self.actions.delete.setEnabled(
                bool(self.label_list.selected_items()) and not drawing_active
            )
    ...
    def _current_image_width(self):
        provider = getattr(getattr(self, "canvas", None), "provider", None)
        if provider is not None:
            return provider.image_width
        if getattr(self, "label_file", None) is not None:
            width = getattr(self.label_file, "image_width", None)
            if width is not None:
                return width
        return self.image.width()

    def _current_image_height(self):
        provider = getattr(getattr(self, "canvas", None), "provider", None)
        if provider is not None:
            return provider.image_height
        if getattr(self, "label_file", None) is not None:
            height = getattr(self.label_file, "image_height", None)
            if height is not None:
                return height
        return self.image.height()

    def _provider_navigator_pixmap(self):
        provider = getattr(self.canvas, "provider", None)
        if provider is None:
            return None

        navigator = self.navigator_dialog.navigator
        size = navigator.size()
        available_w = max(1, size.width() - 20)
        available_h = max(1, size.height() - 20)
        scale = min(
            available_w / provider.image_width,
            available_h / provider.image_height,
            1.0,
        )
        target_w = max(1, int(provider.image_width * scale))
        target_h = max(1, int(provider.image_height * scale))

        from anylabeling.views.labeling.viewport import RectF

        result = provider.read_region(
            RectF(
                0.0,
                0.0,
                float(provider.image_width),
                float(provider.image_height),
            ),
            (target_w, target_h),
        )
        pixmap = result.image
    ...
    def _set_navigator_image(self):
        if not hasattr(self, "navigator_dialog"):
            return False

        provider = getattr(self.canvas, "provider", None)
        if provider is not None:
            pixmap = self._provider_navigator_pixmap()
            if pixmap is None:
                return False
    ...
    def on_navigator_zoom_changed(
        ...
        if self.canvas.camera is not None:
            anchor = None
            if mouse_pos is not None:
                anchor = self._convert_navigator_pos_to_canvas(mouse_pos)
            elif hasattr(self.navigator_dialog, "navigator"):
                nav_widget = self.navigator_dialog.navigator
                if (
                    hasattr(nav_widget, "viewport_rect")
                    and not nav_widget.viewport_rect.isEmpty()
                ):
                    center = nav_widget.viewport_rect.center()
                    anchor = self._convert_navigator_pos_to_canvas(
                        QtCore.QPoint(int(center.x()), int(center.y()))
                    )
            self._set_camera_zoom_percent(zoom_percentage, anchor)
            return
    ...
    def set_zoom(self, value):
        ...
        if self.canvas.camera is not None:
            self._apply_camera_zoom_percent(value)
    ...
    def zoom_request(self, delta, pos):
        if self.canvas.camera is not None:
            ...
            value = int(
                round(self.canvas.camera.coordinate_map().scale * units * 100)
            )
            self._set_camera_zoom_percent(value, pos)
            return
    ...
    @staticmethod
    def _image_for_provider(provider):
        """Return a tiny non-null QImage sentinel for provider-backed images."""
        image = QtGui.QImage(1, 1, QtGui.QImage.Format.Format_RGB32)
        image.fill(Qt.GlobalColor.black)
        return image

    @staticmethod
    def _try_create_image_provider(filename):
        """Return a QImageRegionProvider if *filename* is a large image.

        Uses QImageReader metadata (no full decode) to read dimensions.
        Returns ``None`` when the image is small, the file does not
        exist, or the format is unsupported — callers fall back to the
        full QPixmap path.
        """
        ...
        return QImageRegionProvider(filename)

    def load_file(self, filename=None):  # noqa: C901
        """Load the specified file, or the last opened file if None."""

        # Bump QImageReader allocation limit to support large images
        # (TIFF > 256MB). 0 = unlimited for this reader instance.
        try:
            from PyQt6.QtGui import QImageReader
        ...
        # Decide whether to use provider-backed rendering before any image
        # bytes are loaded. Large-image provider mode keeps image_data empty.
        ...
        if QtCore.QFile.exists(label_file) and LabelFile.is_label_file(
            ...
            try:
                self.label_file = LabelFile(
                    label_file,
                    image_dir,
                    load_image_data=_provider is None,
                )
            except LabelFileError as e:
                self.error_message(
                    self.tr("Error opening file"),
        ...
        if _provider is not None:
            image = self._image_for_provider(_provider)
        else:
        ...
        # Load image into canvas: provider path for large images,
        # full pixmap path for normal images (fallback).
        if _provider is not None:
            self.canvas.load_image_provider(_provider)
        else:
        ...
        if (
            ...
            and (brightness is not None or contrast is not None)
        ):
            self.brightness_contrast_dialog.update_image(
                utils.img_data_to_pil(self.image_data)
            )
    ...
    def resizeEvent(self, _):
        if (
            ...
            and self.canvas.camera is not None
        ):
            self.adjust_scale()
    ...
    def adjust_scale(self, initial=False):
        if self.canvas.camera is not None:
            ...
            self._paint_camera_canvas()
            self.canvas.update()
            # Sync zoom_widget and zoom_values in camera mode
            camera = self.canvas.camera
            if camera is not None:
                value = int(round(camera.coordinate_map().scale * 100))
                self.zoom_widget.setValue(value)
                self.zoom_values[self.filename] = (self.zoom_mode, value)
                if hasattr(self, "navigator_dialog"):
                    self.navigator_dialog.set_zoom_value(value)
    ...
    def _paint_camera_canvas(self):
        camera = self.canvas.set_camera_viewport(
            self.canvas.width(),
            self.canvas.height(),
        )
        if camera is None:
    ...
    def _set_camera_manual_zoom(self):
        if self.canvas.camera is None:
            return
        self.actions.fit_width.setChecked(False)
        self.actions.fit_window.setChecked(False)
        self.zoom_mode = self.MANUAL_ZOOM
        value = int(round(self.canvas.camera.coordinate_map().scale * 100))
        value = max(1, value)
        with QtCore.QSignalBlocker(self.zoom_widget):
            self.zoom_widget.setValue(value)
        self.zoom_values[self.filename] = (self.zoom_mode, value)
        if hasattr(self, "navigator_dialog"):
            self.navigator_dialog.set_zoom_value(value)
    ...
    def _set_camera_zoom_percent(self, value, anchor_view_point=None):
        self.actions.fit_width.setChecked(False)
        self.actions.fit_window.setChecked(False)
        self.zoom_mode = self.MANUAL_ZOOM
        self.zoom_widget.setValue(value)
        ...
        if hasattr(self, "navigator_dialog"):
            self.navigator_dialog.set_zoom_value(value)
        self._apply_camera_zoom_percent(value, anchor_view_point)

    def _apply_camera_zoom_percent(self, value, anchor_view_point=None):
        if self.canvas.camera is None:
            return
        self.canvas.set_camera_scale(value / 100.0, anchor_view_point)
        self.canvas.update()
        self.update_navigator_viewport()
    ...
    def _selected_shapes_for_delete(self):
        selected_shapes = [
            shape
            for shape in self.canvas.selected_shapes
        ...
        for item in self.label_list.selected_items():
            ...
            if (
                ...
                and shape not in selected_shapes
            ):
    ...
    def delete_selected_shape(self):
        if self.canvas.drawing() and self.canvas.current:
            return
        selected_shapes = self._selected_shapes_for_delete()
        if not selected_shapes:
            return
        for shape in self.canvas.selected_shapes:
    ...
    def new_shapes_from_auto_labeling(self, auto_labeling_result):
        """Apply auto labeling results as suggestions (pending user review).

        Phase 3b: AI predictions now enter "suggestion" state instead of
        being committed directly.  Shapes are rendered with dashed outlines
        and a confidence badge.  The user must explicitly accept or reject
        each suggestion.
        """
        ...
        model_name = getattr(auto_labeling_result, "model_name", "AI Model")
        model_id = getattr(auto_labeling_result, "model_id", "unknown")

        # Remove existing suggestions for this asset
        self._clear_pending_suggestions_for_current()

        # Create new suggestions
        suggestions: list[dict] = []
        for shape in auto_labeling_result.shapes:
            suggestion_id = str(uuid.uuid4())
            shape.is_suggestion = True
            shape.suggestion_id = suggestion_id
            shape.suggestion_confidence = getattr(shape, "score", 0.0)

            suggestion = {
                "id": suggestion_id,
                "asset_id": osp.basename(self.image_path),
                "model_id": model_id,
                "model_name": model_name,
                "confidence": getattr(shape, "score", 0.0),
                "shape": shape,
                "status": "pending",
        ...
        # Store suggestions
        self._pending_suggestions[self.image_path] = suggestions

        # Load suggestion shapes onto canvas
        if auto_labeling_result.replace:
            # Clear non-suggestion shapes
            self.load_shapes([], replace=True)
            self.label_list.clear()

        self.load_shapes(
            [s["shape"] for s in suggestions], replace=False
        )
    ...
    def _clear_pending_suggestions_for_current(self) -> None:
        """Remove all suggestion shapes for the current image from canvas."""
        if not self.image_path:
            return
        suggestions = self._pending_suggestions.pop(self.image_path, [])
    ...
    # ------------------------------------------------------------------
    # Phase 3b: Accept / Reject workflow
    # ------------------------------------------------------------------

    def accept_suggestion(self, suggestion_id: str) -> bool:
        """Accept an AI suggestion, converting it to a normal annotation.

        Args:
            suggestion_id: UUID of the suggestion to accept.

        Returns:
            True if the suggestion was found and accepted.
        """
        if not self.image_path:
            return False
        suggestions = self._pending_suggestions.get(self.image_path, [])
        for sug in suggestions:
            if sug["id"] == suggestion_id and sug["status"] == "pending":
                sug["status"] = "accepted"
                shape = sug["shape"]
                shape.is_suggestion = False
                shape.suggestion_id = None
                self.canvas.update()
                self.set_dirty()
                self.suggestion_accepted.emit(suggestion_id)
                return True
        return False

    def reject_suggestion(self, suggestion_id: str) -> bool:
        """Reject an AI suggestion, removing it from the canvas.
    ...
    def accept_all_suggestions(self) -> int:
        """Accept all pending suggestions for the current image.
        ...
        for sug in suggestions:
            if sug["status"] == "pending":
                ...
                shape.is_suggestion = False
                shape.suggestion_id = None
                self.suggestion_accepted.emit(sug["id"])
                count += 1
        if count:
            self.canvas.update()
            self.set_dirty()
    ...
    def reject_all_suggestions(self) -> int:
        """Reject all pending suggestions for the current image.
        ...
        for sug in suggestions:
            if sug["status"] == "pending":
                ...
                if shape in self.canvas.shapes:
                    self.canvas.shapes.remove(shape)
                sug["status"] = "rejected"
                self.suggestion_rejected.emit(sug["id"])
                count += 1
        if count:
            self.canvas.update()
        return count
    ...
    def pending_suggestion_count(self) -> int:
        """Return the number of pending suggestions for the current image."""
        if not self.image_path:
            return 0
        suggestions = self._pending_suggestions.get(self.image_path, [])
        return sum(1 for s in suggestions if s["status"] == "pending")

    def get_pending_suggestions(self) -> list[dict]:
        """Return all pending suggestions for the current image."""
        if not self.image_path:
            return []
        return [
            s for s in self._pending_suggestions.get(self.image_path, [])
            if s["status"] == "pending"
        ]
