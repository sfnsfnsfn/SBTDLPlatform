...
# Fix: prevent anylabeling/platform/ from shadowing stdlib platform module.
# When app.py runs as a script, sys.path[0] is set to the anylabeling/
# directory which contains the platform/ sub-package.  Any subsequent
# `import platform` then finds anylabeling.platform instead of the
# standard-library platform module, breaking callers like darkdetect that
# expect platform.release() to exist.
...
def main():
    ...
    if args.platform:
        ...
        # Optionally open a project from --filename
        if filename and os.path.isdir(filename):
            try:
                from anylabeling.platform.infrastructure.project_file_store import (
                    ProjectFileStore,
                )
                issues = ProjectFileStore.validate_project(filename)
                if not issues:
                    win.set_project(filename)
                else:
                    logger.warning(
                        "Directory %s is not a valid V4 project: %s",
                        filename, "; ".join(issues),
                    )
