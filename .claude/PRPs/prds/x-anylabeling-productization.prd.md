# X-AnyLabeling 离线深度学习平台 — 产品化 UI 整改 PRD
...
- 当前 8 步 Pipeline 导航与实际业务流不匹配，用户无法直观理解数据流转
- 多个页面存在占位控件、静默回退和虚假数据（N/A、伪指标）
- 长任务（导入/构建/训练/导出）阻塞主线程，用户无法离开当前页面
- 标注保存不可靠，切换资产时可能丢失编辑
- 评估指标有重复值和 N/A 填充，无法用于真实决策
- 导出缺少结构化的交付包和自测机制
- 超大图处理链路不完整，切片与标签切分不同步

## Proposed Solution

以 5 域垂直导航替代 8 步水平导航，建立统一的 AppBar + PrimaryNav + PageHeader + StatusBar 外壳骨架，每个页面遵循"一页一个主目标"原则，所有长任务后台化，建立不可变的对象来源链（TaskSpec → DatasetBuild → Run → Evaluation → ModelArtifact），实现完整的离线闭环。

## Key Hypothesis

We believe 5-domain task-driven navigation + unified Shell skeleton + immutable provenance chain will make first-time users able to complete a full training-and-export workflow without external documentation for [离线工业视觉工程师]. We'll know we're right when 8 E2E acceptance tests pass on a clean Windows machine with network disabled.
...
|--------|--------|--------------|
| 完整闭环 | 空项目可完成训练+导出 | E2E-01 ~ E2E-08 全部通过 |
| 首次用户体验 | 无需外部文档走完流程 | UAT 用户观察 |
| 离线可用 | 断网环境全部核心功能可用 | E2E-07 |
| 性能 | 1万资产 5s 可交互，页面切换 200ms | 指定测试机基准测试 |
| 稳定性 | 强制关闭后 manifest 不损坏 | E2E-05 |
| 大图支持 | 32k×32k 浏览+切片链路 | E2E-02 |

## Open Questions

- [ ] Phase 4 中的 TensorRT 导出后端是否在首版实现（当前指定仅 ONNX）
- [ ] 增量训练和模型微调是否在首版支持
- [ ] 命令行无头模式（headless batch processing）是否需要支持
- [ ] 项目文件格式的版本兼容策略（v1/v2 migration 的具体规则）
...
**Primary User**
- **Who**: 工业视觉工程师 / 质检技术员 — 在产线或实验室的 Windows 桌面机上工作，通常无稳定网络连接
- **Current behavior**: 使用多个独立工具串行完成标注、训练、评估和部署，数据在工具间手动拷贝，版本混乱
- **Trigger**: 接到新的产品质检需求 → 需要快速构建检测/分割模型 → 部署到边缘设备或产线 PC
- **Success state**: 从原始图像到一个自测通过的 ONNX 模型包，全程在一个工具内完成，中间产物可追溯、可复现

**Job to Be Done**
When 接到新的工业视觉检测任务，I want to 在一个离线桌面工具中完成数据导入→标注→训练→评估→导出全流程，so that I can 快速交付一个可验证的 ONNX 模型到生产线，而不依赖云服务或外部工具链。

**Non-Users**
- 云端协作的 AI 团队（需要多用户、权限管理、在线标注）
- 纯学术研究人员（需要灵活的实验性框架，不在意产品化交互）
- 非 Windows 用户（首版仅支持 Windows 10/11）
...
|----------|------------|-----------|
| Must | 5 域导航 + 统一 Shell 骨架 | 用户首次使用需直观理解产品结构 |
| Must | 项目新建/打开/最近项目/健康检查/异常恢复 | 项目是工作入口，损坏必须可恢复 |
| Must | 数据导入（文件/文件夹/标注导入/预检/进度/取消） | 闭环起点 |
| Must | 任务配置（检测/分割/旋转框/分类/姿态 + 标签持久化） | 定义模型目标 |
| Must | 标注（基础标注/Polygon/AI预标注入口/自动保存/状态筛选） | 核心工作区 |
| Must | 数据集构建（拆分/切片/增强/不可变 manifest） | 连接标注与训练的桥梁 |
| Must | YOLO 训练（本地模型选择/设备检查/参数预设/运行监控） | 核心价值 |
| Must | 评估（核心指标/每类指标/误判样本/运行来源可见） | 模型质量判断 |
| Must | ONNX 导出（交付包/自测/校验值） | 部署交付 |
| Must | 所有耗时操作后台化 + 统一任务中心 | 不阻塞 UI |
| Must | 离线保障（无账号/无云依赖/无遥测/本地帮助/诊断包） | 离线桌面产品定位 |
| Should | 超大图切片预览与标签同步切分 | 工业常见场景 |
| Should | 推理验证（单图/文件夹/切片推理/合并结果） | 模型验收 |
| Should | WorkflowState（各领域状态和前置条件计算） | 辅助导航 |
| Should | 统一错误组件（技术 traceback 折叠，用户错误前置） | 可用性 |
| Could | 本地模型库（多模型管理） | 多项目复用 |
| Could | 评估报告中的运行对比 | 模型选择 |
| Won't | TensorRT/OpenVINO 导出 | 首版仅 ONNX |
| Won't | 多用户/团队协作 | 单机产品 |
| Won't | 自动超参搜索 | 首版手动配置 |

### MVP Scope

Phase 0–2 + Phase 4 最小集合：项目 → 数据准备（导入/任务/标注/构建）→ 训练 → 评估 → ONNX 导出。可发布为可用 Beta。
...
```
新建项目 → 导入数据(含预检) → 配置任务与标签 → 标注 →
数据集构建(70/20/10拆分) → 训练 → test集评估 → 单图验证 → ONNX导出
```

**超大图流程**（E2E-02）：

```
导入32k TIFF → 配置实例分割 → Polygon/Mask标注 → 640tile+overlap切片 →
标签同步切分 → 训练 → 切片推理 → Mask合并 → 原图结果导出
```
...
### 3.4 Don't Use Disabled as Explanation
...
### 3.5 Long Tasks Background-Side
...
## Page-Level UI & Interaction Specs

### 7.1 Project Center

**Goal**: 让用户在 5 秒内知道"继续什么、打开什么、如何新建"。
...
**Large Image Detection**: 预检读取图像尺寸并标识"超大图"，但不在导入时强制切片。导入结果显示：
...
**Properties Panel** — 仅显示当前选择相关内容：
- 未选择对象：标签统计与图像状态
- 选择对象：标签、几何、属性、可见性、删除
- AI 建议：置信度、接受、拒绝、编辑
- 待复核样本：问题类型和处理结果

**Save Logic** — 所有变化统一触发 `documentChanged`：
- 新增/删除/移动对象
- 修改标签
- 画笔修改 Mask
- 属性修改
...
**Build Completion** — 生成不可变 `DatasetBuild`，页面显示：
...
**Readiness Check** — 开始按钮只有在以下检查通过后启用：
- 数据集构建存在且未损坏
- Provider 支持任务
- 权重可访问且 hash 可计算
- CUDA/CPU 环境可用
...
原始日志放入"详细日志"页签。页面操作：安全停止、保存检查点（后端支持时）、打开输出目录、复制运行配置。没有真实暂停能力时不得显示"暂停"。
...
**Interaction Mode**: 先选择目标环境，再显示格式和参数，禁止首屏出现"格式墙"。
...
**Feasibility**: HIGH — 现有工程已有完整服务层和 Provider 架构，整改主要聚焦 UI 层重构和接线。
...
禁止页面手工拼接松散字典。统一使用 frozen dataclass：
...
#### Existing File Remediation Mapping
...
|------|------------|------------|
| 大爆炸重写导致回归 | Medium | 逐文件迁移，旧代码保留（隐藏）作为回退 |
| PyQt6 vtable 机制导致事件处理陷阱 | Medium | 优先使用 eventFilter 而非 monkey-patch |
| 超大图切片性能不达标 | Medium | 复用现有 TileGrid/TileCache；延迟加载 |
| 万级资产列表卡顿 | Medium | 必须用 QAbstractListModel + 虚拟列表 |
| Windows DPI 缩放裁切 | Low | 使用固定尺寸常量 + 响应式 fold 机制 |
...
|---|-------|-------------|--------|----------|---------|----------|
| 0 | 冻结与基线 | 停止堆功能，建立可重复验收基线 | `awaiting-manual-validation` | - | - | `phase-0-freeze-and-baseline.plan.md` |
| 1 | 真实闭环 | 从空项目走完一次真实流程，所有页面和后端服务真实连接 | `complete` | 拆为 1a/1b/1c | 0 | 1a/1b/1c plans |
| 2 | 新应用外壳与友好交互 | 从"功能拼装"变为"任务驱动产品" | `in-progress` | 拆为 2a/2b | 1 | 2a: ✅ completed |
| 3 | 数据与标注产品化 | 满足真实工业数据使用场景 | `in-progress` | 拆为 3a/3b/3c | 2 | `phase-3-data-and-labeling-productionization.plan.md` |
| 4 | 训练、评估、验证和交付 | 让模型结果可理解、可复现、可部署 | `pending` | 拆为 4a/4b | 3 | - |
| 5 | 发布硬化 | 满足所有发布门槛，可正式交付 | `pending` | 拆为 5a/5b | 4 | - |

### Sub-phase Splitting Thresholds

| Phase | Est. Files | Est. Lines | Split Recommended |
|-------|-----------|-----------|-------------------|
| 0 | ~12 | ~500 | No |
| 1 | 15–25 | ~2000 | **Yes — 1a/1b/1c** |
| 2 | 12–18 | ~1500 | **Yes — 2a/2b** |
| 3 | 15–20 | ~2000 | **Yes — 3a/3b/3c** |
| 4 | 12–18 | ~1800 | **Yes — 4a/4b** |
| 5 | 10–15 | ~1200 | **Yes — 5a/5b** |

### Phase Details

**Phase 0: Freeze & Baseline**
- **Goal**: 停止继续堆功能，建立可重复验收基线
- **Scope**: 冻结产品范围和术语；列出所有可见控件与后端接线状态（审计报告）；未实现控件先隐藏/禁用（含解释性 tooltip）；建立 8 条端到端测试数据；记录当前页面截图和流程录像；建立 Windows 运行环境与打包基线
- **Out of Scope**: 新功能开发；Shell UI 重构（Phase 2）；真实增强管线（Phase 3）；业务逻辑变更；领域模型变更；新依赖引入
- **Success signal**: 审计报告完整；所有未接线控件已禁用/隐藏并有解释；8 个 E2E 测试夹具可运行
...
**Phase 1: Real Closed Loop** ✅
- **Goal**: 从空项目走完一次真实流程，所有页面和后端服务真实连接
- **Scope**: (1a) DataWorkspace 接入 IMPORT 页 + AssetRepository 创建 + InferWorkspace 清理；(1b) Build 按钮修复 + CONFIG 页接入 TaskConfigurator + 参数传递验证；(1c) 来源强绑定审计 + 评估指标修复 + ONNX 基础自测
- **Out of Scope**: 新应用外壳和 5 域导航（Phase 2）；WorkflowState / ProjectSession（Phase 2）；数据导入产品化重构（Phase 3）；标注自动保存和资产虚拟列表（Phase 3）；DPI / 键盘 / 焦点硬化（Phase 5）
- **Completed**: 2026-06-08 — 884 测试通过
...
**Phase 2: New App Shell & Friendly Interaction** 🔄
- **Goal**: 从"功能拼装"变为"任务驱动产品"，首次用户无需了解工程目录即可操作
- **Scope**: (2a) Shell 骨架 ✅ — app_bar, primary_navigation, page_header, status_bar；(2b) WorkflowState + ProjectSession + 任务中心抽屉 + 统一错误组件
- **Out of Scope**: 数据导入预检重构（Phase 3）；标注自动保存和资产虚拟列表（Phase 3）；本地模型库（Phase 4）；训练就绪检查（Phase 4）；DPI / 离线硬化（Phase 5）
- **Success signal**: 一级导航不超过 5 项；每页只有一个主操作按钮；任务中心可看到所有后台任务状态
...
**Phase 3: Data & Labeling Productionization**
- **Goal**: 满足真实工业数据使用场景：万级资产、异常文件、超大图切片
- **Scope**: (3a) 导入预检 + 资产虚拟列表 + 筛选；(3b) 标注自动保存/恢复 + Mask 画笔 + AI 预标注建议；(3c) 大图切片预览 + 标签同步切分抽查 + 构建版本 + manifest + 泄漏检查
- **Out of Scope**: 全自动机器学习大规模搜索；新算法研发；模型市场或在线权重；复杂节点式算法画布
- **Success signal**: 1–5 万张资产可以稳定管理；异常文件不会破坏整个导入流程；标注自动保存覆盖所有编辑类型
...
**Phase 4: Train, Evaluate, Validate, Deliver**
- **Goal**: 让模型结果可理解、可复现、可部署
- **Scope**: (4a) 本地模型库 + 训练就绪检查 + 运行恢复；(4b) 评估报告 + 误判样本 + 推理验证 + 导出交付包
- **Out of Scope**: 除 ONNX 外的其他导出格式（TensorRT、OpenVINO 等首版不做）；新算法研发；云端训练或远程集群
- **Success signal**: 用户可以从评估定位问题 → 回流标注 → 生成新数据集版本 → 重新训练；ONNX 交付包包含所有必需文件
...
- **Goal**: 满足所有发布门槛，可正式交付
- **Scope**: (5a) 环境硬化（中文路径、断网、高DPI、多显示器、GPU 不可用）；(5b) 稳定性硬化（崩溃恢复、性能、安装升级、帮助与诊断包、UAT）
- **Out of Scope**: 新功能；新导出格式；新算法支持
- **Success signal**: 满足全部功能/可用性/稳定性/性能门槛；UAT 通过
...
## First Phase Development Tasks

| ID | Priority | Task | Files/Modules | Est. | Done Criteria |
|---|---|---|---|---|---:|---|
...
参考以下工业视觉平台产品的交互模式和工作流设计：

1. MVTec Deep Learning Tool — Features & Workflows (https://www.mvtec.com/products/deep-learning-tool/features-workflows)
2. 阿丘科技 AIDI — 工业 AI 视觉全周期流程 (https://cn.aqrose.com/aidi.html)
3. Ultralytics YOLO Modes — Train / Val / Predict / Export / Benchmark (https://docs.ultralytics.com/modes/)

### Technical Context

参考以下技术文档的交互规范：

4. Microsoft Windows App Design Guidelines (https://learn.microsoft.com/en-us/windows/apps/design/guidelines-overview)
5. Microsoft Progress Controls Guidance (https://learn.microsoft.com/en-us/windows/apps/develop/ui/controls/progress-controls)
6. Microsoft Error Message Guidelines (https://learn.microsoft.com/en-us/windows/win32/debug/error-message-guidelines)
7. Qt Model/View Programming (https://doc.qt.io/qt-6/model-view-programming.html)
8. Qt High DPI (https://doc.qt.io/qt-6/highdpi.html)
9. Qt Accessibility (https://doc.qt.io/qt-6/accessible.html)

### Existing Codebase Assets

- 完整服务层：ImportService, DatasetBuildService, TrainingService, EvaluationService, InferenceService, ExportService, JobService, ModelRegistryService
- Provider/Capability 插件架构
- TilePlanner, TileMaterializer 和标签 splitters
- 884 个平台测试通过
- Phase 2a Shell 骨架已实现（app_bar, primary_navigation, page_header, status_bar, sub_nav）
...
|----------|--------|--------------|-----------|
| 一级导航 | 5 域 (Project/DataPrep/Train/Eval&Validate/Export) | 8 步 Pipeline / 自选式标签页 | 映射用户心智模型；推理并入评估避免语义重复 |
| 导航实现 | 垂直左侧栏 176px（可折叠 56px） | 水平顶部栏 / Ribbon | 垂直适配宽屏显示器；折叠支持标注沉浸模式 |
| 页面模型 | 一个页面一个主按钮 | 传统多按钮工具栏 | 减少选择负担，引导用户按流程操作 |
| 数据保存 | 不可变版本 + 上游过期标记 | 原地覆盖 + 警告弹窗 | 保证结果可复现、可追溯 |
| 长任务处理 | 全部后台化 + 任务中心抽屉 | 模态进度弹窗 | 用户可切换页面继续工作 |
| 错误呈现 | 用户错误前置 + traceback 折叠 | 直接显示技术异常 | 降低非技术用户的困惑 |
| 离线策略 | 零网络依赖，内置帮助和诊断 | 在线帮助中心 + 遥测 | 产品定位：离线桌面工具 |
| 资产列表 | QAbstractListModel + 虚拟列表 | 创建 N 个 QWidget | 万级资产不卡顿 |
| 导出格式 | 首版仅 ONNX | 多格式同时支持 | 聚焦核心交付物，确保稳定 |

---

*Generated: 2026-06-08*
*Status: DRAFT — needs validation*
*Source: docs/superpowers/plans/X-AnyLabeling_离线深度学习平台_产品化UI整改落地方案.md*
